...
import groovy.time.TimeCategory
  ...
  try {
    ...
    def shipmentsByRuleId = allShipments.flatten().
      groupBy{it.appliedShippingRuleId}.
      findAll{it.value.size() > 100}.
      sort{e1, e2 -> e2.value.size() <=> e1.value.size()}

    //TODO terminar codigo, esta coleccion tiene q ser un mapa de rule y workingHours. Syncronizar logueo de archivo, o sino loguear todo el mapa con resultados, fuera de gpars
    def shippingTimes = Collections.synchronizedList(new ArrayList<Ingeger>())
              
    log("shipmentsByRuleId: ${shipmentsByRuleId.size()}")
    GParsPool.withPool(7) {
      shipmentsByRuleId.eachParallel{ entry ->
        try {
          log("size: ${entry.value.size()}")

          shippingTimes << entry.value.collect { shipment ->
            //TODO usar dateFirstVisit cuando este disponible
            getWorkingHours(shipment.dateShipped, shipment.dateDelivered, siteId)         
          }
  ...
  void log(String line) { //TODO synchronized
