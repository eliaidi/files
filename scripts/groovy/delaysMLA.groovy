...
def filterOddCases(shipmentsAndTimes) {
	shipmentsAndTimes.findAll{it.time >= -15 && it.time <= 15}
}

/** Calcula tiempos de diferencia entre shipping y fecha de entrega (en días hábiles)*/
def calculate(def ships, def speed) {
	def shipmentsAndTimes = ships.collect{
		def estimatedTime = siteEstimatedDeliveryService.getDeliveryDate(
							...
							"MLA", 
							speed[it.shipping_option.shipping_method_id]
						).getTime()
		[
...
def logStats = { shipments, speed -> 
	def firstCalc = calculate(shipments, speed)
...
log "Begin process..."
try {
	[ [73328: 3, 73330: 1], [73328: 5, 73330: 3], [73328: 4, 73330: 2] ].each { speed ->
		csv << "Speed Std,Speed Pri\n"
		csv << "${speed[73328]},${speed[73330]}\n"
		...
		[61,62,63,64].each { servId ->
			GroovyCollections.combinations([["AR-C","AR-B"],["AR-C","AR-B"]]).each{ states ->
				logStats( 
