	    	...
	    	if (order?.status == 'paid') {
				...
				if (payment && ship.cost <= payment.shipping_cost) {
					if (!isRecentlyPaid(payment)) {
						...
						logFile "Payment has been recently approved. Order ${ship.orderId} is ignored"
					}
				
				} else if (payment == null) {
					logFile "Approved payment not found for order ${ship.orderId}"
				} else {
					logFile "Order ${ship.orderId} has status paid with cost ${ship.cost} and payment ${payment?.shipping_cost}"
				}
			
			} else
				logFile "Order ${ship.orderId} has status ${order?.status}"
        }
    } catch (Exception e){
...
def processScriptForSite(siteId) {
	...
	try {
		...
		out << "Getting pending shipments for services...\n"
