...
ocaDelays = new File('/tmp/ocaDelaysOCT.txt').readLines()
meliDelays = new File('/tmp/meliDelaysOCT.csv'); meliDelays.write('')
bothDelays = new File('/tmp/bothDelaysOCT.csv'); bothDelays.write('')
...
void saveDifs(shipsAndSpeeds, serviceId) {
	shipsAndSpeeds.each{ sas ->
		def file
		if (ocaDelays.find{it == sas.shipment.tracking_number})
			file = bothDelays
		else
			file = meliDelays

		if (serviceId in [61L,62L,63L,64L])
			file << "${sas.shipment.tracking_number},${formatDate(sas.begin)},${sas.speed},${formatDate(sas.estimated)},${formatDate(sas.end)},-\n"
		else
			file << "${sas.shipment.tracking_number},${formatDate(sas.begin)},${sas.speed},${formatDate(sas.estimated)},-,${formatDate(sas.end)}\n"
	}
}
...
def calculate(def ships, def serviceId) {
		...
		shipmentsAndTimes = ships.collectParallel {
			...
			if (rule && carrierBeginTime && carrierFinishTime) {
				...
				[
					...
					shipment: it,
					speed: speed,
					begin: carrierBeginTime,
					end: carrierFinishTime,
	...
	saveDifs(_1day, serviceId)
	saveDifs(_2day, serviceId)
	saveDifs(_3day, serviceId)
	saveDifs(_4day, serviceId)
	saveDifs(_5day, serviceId)
...
try {
	...
	meliDelays << "Tracking, Comienzo, Tiempo (+1 día extra), Entrega estimada, Visitado, En sucursal\n"
	bothDelays << "Tracking, Comienzo, Tiempo (+1 día extra), Entrega estimada, Visitado, En sucursal\n"
